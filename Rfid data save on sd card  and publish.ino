...
void loop() {
  ...
  //for store data i need to convert data string to char
  char myCharArray[values.length() + 1];
  values.toCharArray(myCharArray, values.length() + 1);

  createFile("information.txt");
  writeToFile(myCharArray);
  closeFile();
}
...
void publish_data() {
  ...
  while (i < 100) {
    ...
    File file = SD.open("information.txt");
    values = file.readString();
    file.close();
